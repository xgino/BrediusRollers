...
# Import from models
from club_management.models import Address, Club, Profile, Sponsor, TrainingLocation, TrainingTime
from seasonal.models import Team, Role, Coach, Player, Season, Game_day, Game, Training, Score
...
def get_player_goals_by_team(club_name, num):
    ...
    if current_season:
        try:
            ...
            teams = Team.objects.filter(club__name__icontains=club_name)
...
def get_players_in_game(club_name, game_id):
    ...
    if current_season:
        ...
        clubs_with_name = Club.objects.filter(name__icontains=club_name)
...
def get_club_roles():
    current_season = get_current_season()
    if current_season:
        roles = Role.objects.order_by('profile__firstname', 'profile__lastname')
...
def get_club_coaches():
    current_season = get_current_season()
    if current_season:
        coaches = Coach.objects.order_by('profile__firstname', 'profile__lastname')
