# Generated by Django 3.2.13 on 2023-08-20 15:04

import datetime
from django.db import migrations, models
import django.db.models.deletion


class Migration(migrations.Migration):

    initial = True

    dependencies = [
        ('accounts', '0001_initial'),
        ('teams', '0001_initial'),
        ('club', '0001_initial'),
    ]

    operations = [
        migrations.CreateModel(
            name='Game',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('leauge_code', models.CharField(blank=True, max_length=10, null=True, verbose_name='Wedstrijd Code')),
                ('field', models.IntegerField(verbose_name='Veld')),
                ('start_time', models.TimeField(verbose_name='Start tijd')),
                ('end_time', models.TimeField(verbose_name='Eindig tijd')),
                ('home_score', models.IntegerField(blank=True, null=True, verbose_name='Home Goals')),
                ('away_score', models.IntegerField(blank=True, null=True, verbose_name='Away Goals')),
                ('away_team', models.ForeignKey(limit_choices_to={'club__season__end_date__gte': datetime.date(2023, 8, 20), 'club__season__start_date__lte': datetime.date(2023, 8, 20)}, on_delete=django.db.models.deletion.CASCADE, related_name='Away_Team', to='teams.team', verbose_name='Away')),
            ],
        ),
        migrations.CreateModel(
            name='Score',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('goals', models.IntegerField(default=0, verbose_name='Goals')),
                ('assists', models.IntegerField(default=0, verbose_name='assists')),
                ('game', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='games.game', verbose_name='Game')),
                ('player', models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.CASCADE, to='teams.player', verbose_name='Player')),
                ('season', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='club.season', verbose_name='Season')),
            ],
        ),
        migrations.CreateModel(
            name='Game_day',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('sport_hall', models.CharField(max_length=255, verbose_name='Sporthal')),
                ('date', models.DateField(verbose_name='Datum')),
                ('adress', models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.CASCADE, to='accounts.adress', verbose_name='Adres')),
                ('season', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='club.season', verbose_name='Season Moet gelijk zijn aan season van club')),
            ],
        ),
        migrations.AddField(
            model_name='game',
            name='gameday',
            field=models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.CASCADE, to='games.game_day', verbose_name='Game_day'),
        ),
        migrations.AddField(
            model_name='game',
            name='home_team',
            field=models.ForeignKey(limit_choices_to={'club__season__end_date__gte': datetime.date(2023, 8, 20), 'club__season__start_date__lte': datetime.date(2023, 8, 20)}, on_delete=django.db.models.deletion.CASCADE, related_name='Home_Team', to='teams.team', verbose_name='Home'),
        ),
        migrations.AddField(
            model_name='game',
            name='league',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='teams.league', verbose_name='League'),
        ),
    ]
